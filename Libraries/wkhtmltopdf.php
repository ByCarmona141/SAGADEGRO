<?php
    ...
    class Wkhtmltopdf
    {
        ...
        protected $_footerStyleRight = null;
        ...
        public function __construct(array $options = array())
        {
            ...
            if (array_key_exists('print-media-type', $options)) {
                $this->setPrintMediaType($options['print-media-type']);
            }
            if (array_key_exists('run-script', $options)) {
                $this->setRunScript($options['run-script']);
            }
            if (array_key_exists('header-spacing', $options)) {
                $this->setHeaderSpacing($options['header-spacing']);
            }
            if (array_key_exists('header-html', $options)) {
                $this->setHeaderHtml($options['header-html']);
            }
            if (array_key_exists('footer_html', $options)) {
                $this->setFooterHtml($options['footer_html']);
            }
            if (array_key_exists('footer-spacing', $options)) {
                $this->setFooterSpacing($options['footer-spacing']);
            }
        ...
        /*@Mijail Vazquez: funciones para el uso de encabezados html*/
        public function setHeaderHtml($header_html)
        {
            $this->_header_html = (string)$header_html;
            return $this;
        }
        public function setRunScript($run_script)
        {
            $this->_run_script = (string)$run_script;
            return $this;
        }
        public function setPrintMediaType($print_media_type)
        {
            $this->_print_media_type = (boolean)$print_media_type;
            return $this;
        }
        public function setHeaderSpacing($header_spacing)
        {
            $this->_header_spacing = (string)$header_spacing;
            return $this;
        }
        public function setFooterSpacing($footer_spacing)
        {
            $this->_footer_spacing = (string)$footer_spacing;
            return $this;
        }
        ...
        protected function _getCommand()
        {
            ...
            $command .= ($this->getPrintMediaType()) ? " --print-media-type" : "";
            $command .= ($this->getRunScript()) ? " --run-script ".$this->getRunScript()."" : "";

            foreach($this->getMargins() as $position => $margin) {
                $command .= (!is_null($margin)) ? sprintf(' --margin-%s %s', $position, $margin) : '';
            }

            $command .= ($this->getHeaderSpacing()) ? " --header-spacing ".$this->getHeaderSpacing()."" : "";
            $command .= ($this->getHeaderHtml()) ? " --header-html ".$this->getHeaderHtml()."" : "";
            ...
            $command .= ($this->getFooterSpacing() ? " --footer-spacing " . $this->getFooterSpacing() . "" : "");
        ...
        protected function _render()
        {
            ...
            //print_r($content);
            if(substr($content['stdout'], 0, 5) != '%PDF-') {
                if (strpos(mb_strtolower($content['stderr']), 'error')){
                    throw new Exception("System error <pre>" . $content['stderr'] . " ->" . $command . "</pre>");
                }else{
                    throw new Exception("System error <pre>" . $content['stdout'] . " -> " . $command . "</pre>");
                }
            }

            if (mb_strlen($content['stdout'], 'utf-8') === 0) {
                throw new Exception("WKHTMLTOPDF didn't return any data -> " . $command);
            }

            if ((int)$content['return'] > 1) {
                throw new Exception("Shell error, return code: " . (int)$content['return'] . " -> " . $command);
            }
