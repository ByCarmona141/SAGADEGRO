...
#imagen_fondo{
	...
	background: url("http://efectosfiscales.mx/ef_sistema/'.$url_logo.'")  no-repeat center;  
...
<table width="'.$tamanio_dehoja.'" border="1" frame="void">
 ...
 if(!is_null($url_logo))
 	$mihtml .='<img style="height:100px;border:none;" src="http://efectosfiscales.mx/ef_sistema/'.$url_logo.'" />';
 ...
 if($tipocomp=="factura"){   //para el tipo de factura electronica
  ...
  if($tipocomp=="5millar"){   //para el tipo de factura 5 al millar
  $mihtml.= '  
  <table width="'.$tamanio_dehoja.'" border="1" frame="void">
  <tr>
    <td colspan="7" class="encabezado centrado bb br">Importe con letra</td>
  ...
  <tr>
    ...
    <td colspan="2" class="encabezado centrado bb br">Tasa IVA</td>
    <td class="bt bb">&nbsp;</td>
    <td class="encabezado bl bb br">5 al Millar</td>
    <td width="15%" class="valorUnitario bb">$'.number_format(	$monto5millar , $redondeo, '.', ',').'</td>
  </tr>
  <tr>
    <td colspan="3" class="centrado  br">'.$metodopago.'</td>
    <td colspan="2" class="centrado  br">'.$numcuentapago.'</td>
