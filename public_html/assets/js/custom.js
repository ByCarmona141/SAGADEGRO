...
function LoadDataTable(TableId, Sorting, RecordLength, Privileges){
	...
	if(typeof($('#' + TableId).data('source')) == 'undefined'){
		if(typeof($('#' + TableId).data('options')) == 'undefined'){
			Options = {
				responsive: true,
				'sDom': '<"row"<"col-md-12"r>><"row" <"col-md-12 overflow table-responsive" t>><"row" <"col-sm-6"i><"col-sm-6"p>>',
		...
		if(typeof($('#' + TableId).data('options')) == 'undefined'){
			Options = {
				responsive: true,
				'sDom': '<"row text-center " r><"row" <"col-md-12 overflow table-responsive" t>><"row" <"col-sm-6"i><"col-sm-6 "p>>',
				'sPaginationType': 'bootstrap',
				'iDisplayLength' : RecordLength,
				'oLanguage': {
					'sZeroRecords': 'No se encontrarón registros',
	...
	$('.dataTables_processing').each(function(){
		...
		$(this).addClass('alert alert-info alert-dismissible text-center');
		$(this).removeClass('card');

		$(this).css({
			'top': function (){
				return (($content.height() / 2) - ($content.height() / 2) + 250) + 'px';
			},
			'left': function (){
				return (($content.width() / 2) - ($(this).width() / 2) + 150) +  'px';
			},
			'z-index': 10000
		});
	});
		...
		'fnInit': function(oSettings, nPaging, fnDraw) {
			...
			$(nPaging).append(
				'<ul class="pagination" data-intro="Barra de paginaci&oacute;n" data-position="left">'+
					'<li class="paginate_button page-item first disabled"><a class="page-link" href="#">' + oLang.sFirst + '</a></li>' +
					'<li class="paginate_button page-item prev disabled"><a class="page-link" href="#">' + oLang.sPrevious + '</a></li>' +
					'<li class="paginate_button page-item next disabled"><a class="page-link" href="#">' + oLang.sNext + ' </a></li>' +
					'<li class="paginate_button page-item last disabled"><a class="page-link" href="#">' + oLang.sLast + '</a></li>' +
				'</ul>'
			);
			var els = $('a', nPaging);
		...
		'fnUpdate': function (oSettings, fnDraw) {
			...
			for (i = 0, ien = an.length; i < ien; i++) {
				...
				for (j = iStart; j <= iEnd; j++) {
					sClass = (j == oPaging.iPage + 1) ? 'class="paginate_button page-item active middle"' : 'class="paginate_button page-item middle"';
...
$(function(){
	/*Cookie menu hidden*/
	if(typeof $.cookie('app-sidebar-minified') == 'undefined'){
		$.cookie('app-sidebar-minified', 0);
	}

	if($.cookie('app-sidebar-minified') == true){
...
$(document).ready(function(){
	...
	$(function () {
		...
		$('input[type=file]').before(
			'<button style="z-index: 4;" class="btn-close limpiar-inputfile" type="button" title="Quitar archivo"> <span aria-hidden="true"></span><span class="sr-only">Close</span></button>'
		);
