<?php
	 ...
	 * Fecha: 12/11/2015
	 * Descripción: Controlador de la tabla "CelaZonaHoraria".
	 **/

	require_once('../Libraries/Functions.php');
	require_once('../Libraries/Session.class.php');
	require_once('../CelaZonaHoraria/CelaZonaHoraria.php');
	require_once('../Libraries/Connection.php');
	require_once('../Libraries/GetSession.php');
	require_once('../Libraries/Security.php');

	$ArgsCelaZonaHorariaVistaLeer = array(
		'Table'             => 'CelaZonaHoraria',
		'ServerSource'      => '../CelaZonaHoraria/CelaZonaHoraria.php',
		'ServerFunction'    => 'CelaZonaHorariaLeer',
		'RouteForm'         => $RouteForm
	);

	$ArgsCelaHeadContent['FormTitle'] = 'Listado de Zonas Horarias';
		...
		switch($_GET['Action']){
			case 'Crear':
				...
				if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Zona Horaria';

					/*Se verifica que haya evento "submit"*/
					if((isset($_POST['CelaZonaHorariaInsert'])) && ($_POST['CelaZonaHorariaInsert'] == 'CelaZonaHorariaInsert')){
						/*Se invoca la funcion crear*/
						$CelaZonaHoraria = CelaZonaHorariaCrear($_POST);

						if($CelaZonaHoraria['Status'] == 'OK'){
							/*Se registra la acción "Crear" en la bitacora*/
							RecordLog('CelaZonaHoraria', $CelaZonaHoraria['idRecord'], 2, $SessionUserId, $_POST);
							$Status = true;
							...
							if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
								...
								$ArgsCelaZonaHorariaVistaCrear = array(
									'SessionGroupId'    => $SessionGroupId,
									'Random'            => $SessionRandom,
									'FormAction'        => $RouteForm
								);

								$Content .= LoadContentPage('../CelaZonaHoraria/CelaZonaHorariaVistaCrear.php', $ArgsCelaZonaHorariaVistaCrear);
							}else{
								/*Se carga la vista de Lectura*/
								if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
									$Content  .= LoadContentPage('../CelaZonaHoraria/CelaZonaHorariaVistaLeer.php', $ArgsCelaZonaHorariaVistaLeer);

									$ArgsCelaZonaHorariaJavascript = array(
										'Table' => 'CelaZonaHoraria',
									);

									$MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsCelaZonaHorariaJavascript);
								}
							}
						}else{
							$Status = false;
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => $CelaZonaHoraria['Error'].'<br />puede <a href="CelaZonaHoraria.php?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
							);
			...
			case 'Eliminar':
				...
				if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
					...
					foreach ($_GET['Key'] as $Key) {
						...
						$Data = GetValue(
									sprintf('SELECT * FROM CelaZonaHoraria WHERE `id` = %s;',
										GetSQLValueString($Key, 'int')
									)
								);
						$CelaZonaHoraria = CelaZonaHorariaEliminar($Key);

						if($CelaZonaHoraria['Status'] == 'ERROR'){
							/*Se guarda el error para mostrarlo*/
							$Status = false;
							$Result = array();

							$Result['Index']    = $Key;
							$Result['Error']    = $CelaZonaHoraria['Error'];
							$BadKeys[]          = $Result;
						}else{
							/*Se registra la acción "Eliminar" en la bitacora*/
							RecordLog('CelaZonaHoraria', $Key, 3, $SessionUserId, $Data);
						}
					}
					if($Status){
						...
						$ArgsCelaActionMessage['TextMessage'] .= '<a href="CelaZonaHoraria.php"
	class="btn btn-danger">Aceptar</a>';
					...
					$Connection -> close();
					header(sprintf('Location: %s', 'CelaZonaHoraria.php'));
				}
				break;
			case 'Actualizar':
				/*Se verifica si se tiene el privilegio de actualizar*/
				if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Zona Horaria';

					/*Se verifica que haya evento "submit"*/
					if(isset($_POST['CelaZonaHorariaUpdate']) && $_POST['CelaZonaHorariaUpdate'] == 'CelaZonaHorariaUpdate'){
						...
						if($Status){
							...
							foreach($Keys as $Key){
								...
								$CelaZonaHoraria = CelaZonaHorariaActualizar($Key, $Data);
								...
								if($CelaZonaHoraria['Status'] == 'ERROR'){
									...
									RecordLog('CelaZonaHoraria', $Key, 5, $SessionUserId, $Data);
								}
							}
	...
	class="btn btn-danger">Aceptar</a>';
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="CelaZonaHoraria.php"
	class="btn btn-danger">Aceptar</a>'
							);
							$Content = LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
						}
						...
						if(isset($_GET['Key']) && $_GET['Key'] != '') {
							...
							$ArgsCelaZonaHorariaVistaActualizar = array(
								'SessionGroupId'    => $SessionGroupId,
								'Random'            => $SessionRandom,
								'FormAction'        => $RouteForm
							);
							$Content .= LoadContentPage('../CelaZonaHoraria/CelaZonaHorariaVistaActualizar.php', $ArgsCelaZonaHorariaVistaActualizar);
						}else{
