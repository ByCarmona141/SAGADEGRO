...
$ArgsTipoDispositivoVistaLeer = array(
    'Table'             => 'TipoDispositivo',
    'ServerSource'      => '../TipoDispositivo/TipoDispositivo.php',
    'ServerFunction'    => 'TipoDispositivoLeer',
    'RouteForm'         => $RouteForm
);

$ArgsCelaHeadContent['FormTitle'] = 'Listado de Tipo de Dispositivo';
...
if(isset($_GET['Action'])) {
    switch($_GET['Action']) {
        case 'Crear':
            ...
            if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Tipo de Dispositivo';

                /*Se verifica que haya evento "submit"*/
                if((isset($_POST['TipoDispositivoInsert'])) && ($_POST['TipoDispositivoInsert'] == 'TipoDispositivoInsert')) {
                    /*Se invoca la funcion crear*/
                    $Data = array(
                        'Nombre' => $_POST['Nombre'],
                        'Icono' => $_POST['Icono']
                    );
                    $TipoDispositivo = TipoDispositivoCrear($Data);

                    if($TipoDispositivo['Status'] == 'OK') {
                        /*Se registra la acción "Crear" en la bitacora*/
                        RecordLog('TipoDispositivo', $TipoDispositivo['idRecord'], 2, $SessionUserId, $Data);
                        $Status = true;
                        ...
                        if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1) {
                            ...
                            $ArgsTipoDispositivoVistaCrear = array(
                                'SessionGroupId'    => $SessionGroupId,
                                'Random'            => $SessionRandom,
                                'FormAction'        => $RouteForm
                            );

                            $Content .= LoadContentPage('../TipoDispositivo/TipoDispositivoCrear.php', $ArgsTipoDispositivoVistaCrear);
                        } else {
                            /*Se carga la vista de Leer*/
                            if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
                                $ArgsCelaHeadContent['FormTitle'] = 'Listado de Tipo de Dispositivos';
                                $Content  .= LoadContentPage('../TipoDispositivo/TipoDispositivoLeer.php', $ArgsTipoDispositivoVistaLeer);

                                $ArgsTipoDispositivoJavascript = array(
                                    'Table' => 'TipoDispositivo',
                                );

                                $MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsTipoDispositivoJavascript);
                            }
                        }
                    } else {
                        $Status = false;
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => $TipoDispositivo['Error'].'<br />puede <a href="TipoDispositivo?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
                        );
        ...
        case 'Eliminar':
            ...
            if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
                ...
                foreach ($_GET['Key'] as $Key) {
                    ...
                    $Data = GetValue(
                        sprintf('SELECT * FROM TipoDispositivo WHERE `id` = %s;',
                            GetSQLValueString($Key, 'tinyint unsigned')
                        )
                    );
                    $TipoDispositivo = TipoDispositivoEliminar($Key);

                    if($TipoDispositivo['Status'] == 'ERROR'){
                        /*Se guarda el error para mostrarlo*/
                        $Status = false;
                        $Result = array(
                            'Index' => $Key,
                            'Error' => $TipoDispositivo['Error']
                        );

                        $BadKeys[] = $Result;
                    }else{
                        /*Se registra la acción "Eliminar" en la bitacora*/
                        RecordLog('TipoDispositivo', $Key, 3, $SessionUserId, $Data);
                    }
                }
                if($Status) {
                    ...
                    if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
                        $ArgsCelaHeadContent['FormTitle'] = 'Listado de TipoDispositivo de Dispositivos';
                        $Content .= LoadContentPage('../TipoDispositivo/TipoDispositivoLeer.php', $ArgsTipoDispositivoVistaLeer);
                    ...
                    $ArgsCelaActionMessage['TextMessage'] .= '<a href="TipoDispositivo"
	class="btn btn-danger">Aceptar</a>';
                ...
                $Connection -> close();
                header(sprintf('Location: %s', 'TipoDispositivo'));
            }
            break;
        case 'Actualizar':
            /*Se verifica si se tiene el privilegio de actualizar*/
            if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de TipoDispositivo de Dispositivos';

                /*Se verifica que haya evento "submit"*/
                if(isset($_POST['TipoDispositivoUpdate']) && $_POST['TipoDispositivoUpdate'] == 'TipoDispositivoUpdate'){
                    ...
                    if($Status) {
                        ...
                        foreach($Keys as $Key) {
                            ...
                            $TipoDispositivo = TipoDispositivoActualizar($Key, $Data);
                            ...
                            if($TipoDispositivo['Status'] == 'ERROR') {
                                ...
                                RecordLog('TipoDispositivo', $Key, 5, $SessionUserId, $Data);
                            }
                        }
	...
	class="btn btn-danger">Aceptar</a>';
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="TipoDispositivo"
	class="btn btn-danger">Aceptar</a>'
                        );
                        $Content .= LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
                    }
                    ...
                    if(isset($_GET['Key']) && $_GET['Key'] != '') {
                        ...
                        $ArgsTipoDispositivoVistaActualizar = array(
                            'SessionGroupId'    => $SessionGroupId,
                            'Random'            => $SessionRandom,
                            'FormAction'        => $RouteForm
                        );
                        $Content .= LoadContentPage('../TipoDispositivo/TipoDispositivoActualizar.php', $ArgsTipoDispositivoVistaActualizar);
                    }else{
