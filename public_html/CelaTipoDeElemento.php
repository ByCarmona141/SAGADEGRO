<?php
	 ...
	 * Fecha: 12/11/2015
	 * Descripción: Controlador de la tabla "CelaTipoDeElemento".
	 **/
	...
	$ArgsCelaTipoDeElementoVistaLeer = array(
		'Table'             => 'CelaTipoDeElemento',
		'ServerSource'      => '../CelaTipoDeElemento/CelaTipoDeElemento.php',
		'ServerFunction'    => 'CelaTipoDeElementoLeer',
		'RouteForm'         => $RouteForm
	);

	$ArgsCelaHeadContent['FormTitle'] = 'Listado de Tipo de Elemento';
		...
		switch($_GET['Action']){
			case 'Crear':
				...
				if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de CelaZonaHoraria';

					/*Se verifica que haya evento "submit"*/
					if((isset($_POST['CelaTipoDeElementoInsert'])) && ($_POST['CelaTipoDeElementoInsert'] == 'CelaTipoDeElementoInsert')){
						/*Se invoca la funcion crear*/
						$CelaTipoDeElemento = CelaTipoDeElementoCrear($_POST);

						if($CelaTipoDeElemento['Status'] == 'OK'){
							/*Se registra la acción "Crear" en la bitacora*/
							RecordLog('CelaTipoDeElemento', $CelaTipoDeElemento['idRecord'], 2, $SessionUserId, $_POST);
							$Status = true;
							...
							if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
								...
								$ArgsCelaTipoDeElementoVistaCrear = array(
									'SessionGroupId'    => $SessionGroupId,
									'Random'            => $SessionRandom,
									'FormAction'        => $RouteForm
								);

								$Content .= LoadContentPage('../CelaTipoDeElemento/CelaTipoDeElementoVistaCrear.php', $ArgsCelaTipoDeElementoVistaCrear);
							}else{
								if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
									$Content  .= LoadContentPage('../CelaTipoDeElemento/CelaTipoDeElementoVistaLeer.php', $ArgsCelaTipoDeElementoVistaLeer);

									$ArgsCelaTipoDeElementoJavascript = array(
										'Table' => 'CelaTipoDeElemento',
									);

									$MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsCelaTipoDeElementoJavascript);
								}
							}
						}else{
							$Status = false;
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => $CelaTipoDeElemento['Error'].'<br />puede <a href="CelaTipoDeElemento.php?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
							);
			...
			case 'Eliminar':
				...
				if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
					...
					foreach ($_GET['Key'] as $Key) {
						...
						$Data = GetValue(
									sprintf('SELECT * FROM CelaTipoDeElemento WHERE `id` = %s;',
										GetSQLValueString($Key, 'int')
									)
								);
						$CelaTipoDeElemento = CelaTipoDeElementoEliminar($Key);

						if($CelaTipoDeElemento['Status'] == 'ERROR'){
							/*Se guarda el error para mostrarlo*/
							$Status = false;
							$Result = array();

							$Result['Index']    = $Key;
							$Result['Error']    = $CelaTipoDeElemento['Error'];
							$BadKeys[]          = $Result;
						}else{
							/*Se registra la acción "Eliminar" en la bitacora*/
							RecordLog('CelaTipoDeElemento', $Key, 3, $SessionUserId, $Data);
						}
					}
					if($Status){
						...
						$ArgsCelaActionMessage['TextMessage'] .= '<a href="CelaTipoDeElemento.php"
	class="btn btn-danger">Aceptar</a>';
					...
					$Connection -> close();
					header(sprintf('Location: %s', 'CelaTipoDeElemento.php'));
				}
				break;
			case 'Actualizar':
				/*Se verifica si se tiene el privilegio de actualizar*/
				if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de CelaZonaHoraria';

					/*Se verifica que haya evento "submit"*/
					if(isset($_POST['CelaTipoDeElementoUpdate']) && $_POST['CelaTipoDeElementoUpdate'] == 'CelaTipoDeElementoUpdate'){
						...
						if($Status){
							...
							foreach($Keys as $Key){
								...
								$CelaTipoDeElemento = CelaTipoDeElementoActualizar($Key, $Data);
								...
								if($CelaTipoDeElemento['Status'] == 'ERROR'){
									...
									RecordLog('CelaTipoDeElemento', $Key, 5, $SessionUserId, $Data);
								}
							}
	...
	class="btn btn-danger">Aceptar</a>';
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="CelaTipoDeElemento.php"
	class="btn btn-danger">Aceptar</a>'
							);
							$Content = LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
						}
						...
						if(isset($_GET['Key']) && $_GET['Key'] != '') {
							...
							$ArgsCelaTipoDeElementoVistaActualizar = array(
								'SessionGroupId'    => $SessionGroupId,
								'Random'            => $SessionRandom,
								'FormAction'        => $RouteForm
							);
							$Content .= LoadContentPage('../CelaTipoDeElemento/CelaTipoDeElementoVistaActualizar.php', $ArgsCelaTipoDeElementoVistaActualizar);
						}else{
