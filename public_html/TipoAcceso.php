...
$ArgsTipoAccesoVistaLeer = array(
    'Table'             => 'TipoAcceso',
    'ServerSource'      => '../TipoAcceso/TipoAcceso.php',
    'ServerFunction'    => 'TipoAccesoLeer',
    'RouteForm'         => $RouteForm
);

$ArgsCelaHeadContent['FormTitle'] = 'Listado de Tipos de Acceso';
...
if(isset($_GET['Action'])) {
    switch($_GET['Action']) {
        case 'Crear':
            ...
            if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Tipos de Accesos';

                /*Se verifica que haya evento "submit"*/
                if((isset($_POST['TipoAccesoInsert'])) && ($_POST['TipoAccesoInsert'] == 'TipoAccesoInsert')) {
                    /*Se invoca la funcion crear*/
                    $Data = array(
                        'Nombre' => $_POST['Nombre']
                    );
                    $TipoAcceso = TipoAccesoCrear($Data);

                    if($TipoAcceso['Status'] == 'OK') {
                        /*Se registra la acción "Crear" en la bitacora*/
                        RecordLog('TipoAcceso', $TipoAcceso['idRecord'], 2, $SessionUserId, $Data);
                        $Status = true;
                        ...
                        if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
                            ...
                            $ArgsTipoAccesoVistaCrear = array(
                                'SessionGroupId'    => $SessionGroupId,
                                'Random'            => $SessionRandom,
                                'FormAction'        => $RouteForm
                            );

                            $Content .= LoadContentPage('../TipoAcceso/TipoAccesoCrear.php', $ArgsTipoAccesoVistaCrear);
                        }else{
                            ...
                            if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
                                ...
                                $Content  .= LoadContentPage('../TipoAcceso/TipoAccesoLeer.php', $ArgsTipoAccesoVistaLeer);

                                $ArgsTipoAccesoJavascript = array(
                                    'Table' => 'TipoAcceso',
                                );

                                $MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsTipoAccesoJavascript);
                            }
                        }
                    }else{
                        $Status = false;
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => $TipoAcceso['Error'].'<br />puede <a href="TipoAcceso?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
                        );
        ...
        case 'Eliminar':
            ...
            if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
                ...
                foreach ($_GET['Key'] as $Key) {
                    ...
                    $Data = GetValue(
                        sprintf('SELECT * FROM TipoAcceso WHERE `id` = %s;',
                            GetSQLValueString($Key, 'tinyint unsigned')
                        )
                    );
                    $TipoAcceso = TipoAccesoEliminar($Key);

                    if($TipoAcceso['Status'] == 'ERROR'){
                        /*Se guarda el error para mostrarlo*/
                        $Status = false;
                        $Result = array(
                            'Index' => $Key,
                            'Error' => $TipoAcceso['Error']
                        );

                        $BadKeys[] = $Result;
                    }else{
                        /*Se registra la acción "Eliminar" en la bitacora*/
                        RecordLog('TipoAcceso', $Key, 3, $SessionUserId, $Data);
                    }
                }
                if($Status){
                    ...
                    $ArgsCelaActionMessage['TextMessage'] .= '<a href="TipoAcceso"
	class="btn btn-danger">Aceptar</a>';
                ...
                $Connection -> close();
                header(sprintf('Location: %s', 'TipoAcceso'));
            }
            break;
        case 'Actualizar':
            /*Se verifica si se tiene el privilegio de actualizar*/
            if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Tipos de Accesos';

                /*Se verifica que haya evento "submit"*/
                if(isset($_POST['TipoAccesoUpdate']) && $_POST['TipoAccesoUpdate'] == 'TipoAccesoUpdate'){
                    ...
                    if($Status) {
                        ...
                        foreach($Keys as $Key) {
                            ...
                            $TipoAcceso = TipoAccesoActualizar($Key, $Data);
                            ...
                            if($TipoAcceso['Status'] == 'ERROR') {
                                ...
                                RecordLog('TipoAcceso', $Key, 5, $SessionUserId, $Data);
                            }
                        }
	...
	class="btn btn-danger">Aceptar</a>';
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="TipoAcceso"
	class="btn btn-danger">Aceptar</a>'
                        );
                        $Content .= LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
                    }
                    ...
                    if(isset($_GET['Key']) && $_GET['Key'] != '') {
                        ...
                        $ArgsTipoAccesoVistaActualizar = array(
                            'SessionGroupId'    => $SessionGroupId,
                            'Random'            => $SessionRandom,
                            'FormAction'        => $RouteForm
                        );
                        $Content .= LoadContentPage('../TipoAcceso/TipoAccesoActualizar.php', $ArgsTipoAccesoVistaActualizar);
                    }else{
