...
$ArgsEstatusVistaLeer = array(
    'Table'             => 'Estatus',
    'ServerSource'      => '../Estatus/Estatus.php',
    'ServerFunction'    => 'EstatusLeer',
    'RouteForm'         => $RouteForm
);

$ArgsCelaHeadContent['FormTitle'] = 'Listado de Estatus de Dispositivos';
...
if(isset($_GET['Action'])) {
    switch($_GET['Action']) {
        case 'Crear':
            ...
            if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Estatus de Dispositivos';

                /*Se verifica que haya evento "submit"*/
                if((isset($_POST['EstatusInsert'])) && ($_POST['EstatusInsert'] == 'EstatusInsert')) {
                    /*Se invoca la funcion crear*/
                    $Data = array(
                        'Nombre' => $_POST['Nombre']
                    );
                    $Estatus = EstatusCrear($Data);

                    if($Estatus['Status'] == 'OK') {
                        /*Se registra la acción "Crear" en la bitacora*/
                        RecordLog('Estatus', $Estatus['idRecord'], 2, $SessionUserId, $Data);
                        $Status = true;
                        ...
                        if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
                            ...
                            $ArgsEstatusVistaCrear = array(
                                'SessionGroupId'    => $SessionGroupId,
                                'Random'            => $SessionRandom,
                                'FormAction'        => $RouteForm
                            );

                            $Content .= LoadContentPage('../Estatus/EstatusCrear.php', $ArgsEstatusVistaCrear);
                        }else{
                            /*Se carga la vista de Leer*/
                            if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
                                $ArgsCelaHeadContent['FormTitle'] = 'Listado de Estatus de Dispositivos';
                                $Content  .= LoadContentPage('../Estatus/EstatusLeer.php', $ArgsEstatusVistaLeer);

                                $ArgsEstatusJavascript = array(
                                    'Table' => 'Estatus',
                                );

                                $MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsEstatusJavascript);
                            }
                        }
                    }else{
                        $Status = false;
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => $Estatus['Error'].'<br />puede <a href="Estatus?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
                        );
        ...
        case 'Eliminar':
            ...
            if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
                ...
                foreach ($_GET['Key'] as $Key) {
                    ...
                    $Data = GetValue(
                        sprintf('SELECT * FROM Estatus WHERE `id` = %s;',
                            GetSQLValueString($Key, 'tinyint unsigned')
                        )
                    );
                    $Estatus = EstatusEliminar($Key);

                    if($Estatus['Status'] == 'ERROR'){
                        ...
                        RecordLog('Estatus', $Key, 3, $SessionUserId, $Data);
                    }
                }
                if($Status){
                    ...
                    $ArgsCelaActionMessage['TextMessage'] .= '<a href="Estatus"
	class="btn btn-danger">Aceptar</a>';
                ...
                $Connection -> close();
                header(sprintf('Location: %s', 'Estatus'));
            }
            break;
        case 'Actualizar':
            /*Se verifica si se tiene el privilegio de actualizar*/
            if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Estatus de Dispositivos';

                /*Se verifica que haya evento "submit"*/
                if(isset($_POST['EstatusUpdate']) && $_POST['EstatusUpdate'] == 'EstatusUpdate'){
                    ...
                    if($Status) {
                        ...
                        foreach($Keys as $Key) {
                            ...
                            $Estatus = EstatusActualizar($Key, $Data);
                            ...
                            if($Estatus['Status'] == 'ERROR') {
                                ...
                                RecordLog('Estatus', $Key, 5, $SessionUserId, $Data);
                            }
                        }
                        ...
                        if($Status){
                            ...
                            if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
                                $ArgsCelaHeadContent['FormTitle'] = 'Listado de Estatus';
                                $Content .= LoadContentPage('../Estatus/EstatusLeer.php', $ArgsEstatusVistaLeer);
	...
	class="btn btn-danger">Aceptar</a>';
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="Estatus"
	class="btn btn-danger">Aceptar</a>'
                        );
                        $Content .= LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
                    }
                    ...
                    if(isset($_GET['Key']) && $_GET['Key'] != '') {
                        ...
                        $ArgsEstatusVistaActualizar = array(
                            'SessionGroupId'    => $SessionGroupId,
                            'Random'            => $SessionRandom,
                            'FormAction'        => $RouteForm
                        );
                        $Content .= LoadContentPage('../Estatus/EstatusActualizar.php', $ArgsEstatusVistaActualizar);
                    }else{
