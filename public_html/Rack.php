...
$ArgsRackVistaLeer = array(
    'Table'             => 'Rack',
    'ServerSource'      => '../Rack/Rack.php',
    'ServerFunction'    => 'RackLeer',
    'RouteForm'         => $RouteForm
);

$ArgsCelaHeadContent['FormTitle'] = 'Listado de Racks';
...
if(isset($_GET['Action'])) {
    switch($_GET['Action']) {
        case 'Crear':
            ...
            if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Rack';

                /*Se verifica que haya evento "submit"*/
                if((isset($_POST['RackInsert'])) && ($_POST['RackInsert'] == 'RackInsert')) {
                    ...
                    if($Rack['Status'] == 'OK') {
                        ...
                        RecordLog('Rack', $Rack['idRecord'], 2, $SessionUserId, $Data);
                        $Status = true;
                        ...
                        if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
                            ...
                            $ArgsRackVistaCrear = array(
                                'SessionGroupId'    => $SessionGroupId,
                                'Random'            => $SessionRandom,
                                'FormAction'        => $RouteForm
                            );

                            $Content .= LoadContentPage('../Rack/ModeloCrear.php', $ArgsRackVistaCrear);
                        }else{
                            /*Se carga la vista de Leer*/
                            if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
                                $ArgsCelaHeadContent['FormTitle'] = 'Listado de Racks';
                                $Content  .= LoadContentPage('../Rack/ModeloLeer.php', $ArgsRackVistaLeer);

                                $ArgsRackJavascript = array(
                                    'Table' => 'Rack',
                                );

                                $MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsRackJavascript);
                            }
                        }
                    }else{
                        $Status = false;
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => $Rack['Error'].'<br />puede <a href="Rack?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
                        );
        ...
        case 'Eliminar':
            ...
            if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
                ...
                foreach ($_GET['Key'] as $Key) {
                    ...
                    $Data = GetValue(
                        sprintf('SELECT * FROM Rack WHERE `id` = %s;',
                            GetSQLValueString($Key, 'tinyint unsigned')
                        )
                    );
                    $Rack = RackEliminar($Key);
                    ...
                    if($Rack['Status'] == 'ERROR'){
                        ...
                        RecordLog('Rack', $Key, 3, $SessionUserId, $Data);
                    }
                }
                if($Status){
                    ...
                    $ArgsCelaActionMessage['TextMessage'] .= '<a href="Rack"
	class="btn btn-danger">Aceptar</a>';
                ...
                $Connection -> close();
                header(sprintf('Location: %s', 'Rack'));
            }
            break;
        case 'Actualizar':
            /*Se verifica si se tiene el privilegio de actualizar*/
            if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Racks';

                /*Se verifica que haya evento "submit"*/
                if(isset($_POST['RackUpdate']) && $_POST['RackUpdate'] == 'RackUpdate'){
                    ...
                    if($Status) {
                        ...
                        foreach($Keys as $Key) {
                            ...
                            if($Rack['Status'] == 'ERROR') {
                                ...
                                RecordLog('Rack', $Key, 5, $SessionUserId, $Data);
                            }
                        }
	...
	class="btn btn-danger">Aceptar</a>';
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="Rack"
	class="btn btn-danger">Aceptar</a>'
                        );
                        $Content .= LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
                    }
                    ...
                    if(isset($_GET['Key']) && $_GET['Key'] != '') {
                        ...
                        $ArgsRackVistaActualizar = array(
                            'SessionGroupId'    => $SessionGroupId,
                            'Random'            => $SessionRandom,
                            'FormAction'        => $RouteForm
                        );
                        $Content .= LoadContentPage('../Rack/RackActualizar.php', $ArgsRackVistaActualizar);
                    }else{
