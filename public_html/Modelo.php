...
$ArgsModeloVistaLeer = array(
    'Table'             => 'Modelo',
    'ServerSource'      => '../Modelo/Modelo.php',
    'ServerFunction'    => 'ModeloLeer',
    'RouteForm'         => $RouteForm
);

$ArgsCelaHeadContent['FormTitle'] = 'Listado de Modelos';
...
if(isset($_GET['Action'])) {
    switch($_GET['Action']) {
        case 'Crear':
            ...
            if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Modelo';

                /*Se verifica que haya evento "submit"*/
                if((isset($_POST['ModeloInsert'])) && ($_POST['ModeloInsert'] == 'ModeloInsert')) {
                    /*Se invoca la funcion crear*/
                    $Data = array(
                        'Marca' => $_POST['Marca'],
                        'Nombre' => strtoupper($_POST['Nombre'])
                    );
                    $Modelo = ModeloCrear($Data);

                    if($Modelo['Status'] == 'OK') {
                        /*Se registra la acción "Crear" en la bitacora*/
                        RecordLog('Modelo', $Modelo['idRecord'], 2, $SessionUserId, $Data);
                        $Status = true;
                        ...
                        if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
                            ...
                            $ArgsModeloVistaCrear = array(
                                'SessionGroupId'    => $SessionGroupId,
                                'Random'            => $SessionRandom,
                                'FormAction'        => $RouteForm
                            );

                            $Content .= LoadContentPage('../Modelo/ModeloCrear.php', $ArgsModeloVistaCrear);
                        }else{
                            /*Se carga la vista de Leer*/
                            if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
                                $ArgsCelaHeadContent['FormTitle'] = 'Listado de Modelos';
                                $Content  .= LoadContentPage('../Modelo/ModeloLeer.php', $ArgsModeloVistaLeer);

                                $ArgsModeloJavascript = array(
                                    'Table' => 'Modelo',
                                );

                                $MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsModeloJavascript);
                            }
                        }
                    }else{
                        $Status = false;
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => $Modelo['Error'].'<br />puede <a href="Modelo?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
                        );
        ...
        case 'Eliminar':
            ...
            if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
                ...
                foreach ($_GET['Key'] as $Key) {
                    ...
                    $Data = GetValue(
                        sprintf('SELECT * FROM Modelo WHERE `id` = %s;',
                            GetSQLValueString($Key, 'tinyint unsigned')
                        )
                    );
                    $Modelo = ModeloEliminar($Key);

                    if($Modelo['Status'] == 'ERROR'){
                        ...
                        RecordLog('Modelo', $Key, 3, $SessionUserId, $Data);
                    }
                }
                if($Status){
                    ...
                    $ArgsCelaActionMessage['TextMessage'] .= '<a href="Modelo"
	class="btn btn-danger">Aceptar</a>';
                ...
                $Connection -> close();
                header(sprintf('Location: %s', 'Modelo'));
            }
            break;
        case 'Actualizar':
            /*Se verifica si se tiene el privilegio de actualizar*/
            if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Modelos';

                /*Se verifica que haya evento "submit"*/
                if(isset($_POST['ModeloUpdate']) && $_POST['ModeloUpdate'] == 'ModeloUpdate'){
                    ...
                    if($Status) {
                        ...
                        foreach($Keys as $Key) {
                            ...
                            $Data = array(
                                ...
                                'Nombre' => strtoupper($_POST['Nombre' . $Key])
                            );
                            $Modelo = ModeloActualizar($Key, $Data);
                            ...
                            if($Modelo['Status'] == 'ERROR') {
                                ...
                                RecordLog('Modelo', $Key, 5, $SessionUserId, $Data);
                            }
                        }
	...
	class="btn btn-danger">Aceptar</a>';
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="Modelo"
	class="btn btn-danger">Aceptar</a>'
                        );
                        $Content .= LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
                    }
                    ...
                    if(isset($_GET['Key']) && $_GET['Key'] != '') {
                        ...
                        $ArgsModeloVistaActualizar = array(
                            'SessionGroupId'    => $SessionGroupId,
                            'Random'            => $SessionRandom,
                            'FormAction'        => $RouteForm
                        );
                        $Content .= LoadContentPage('../Modelo/ModeloActualizar.php', $ArgsModeloVistaActualizar);
                    }else{
