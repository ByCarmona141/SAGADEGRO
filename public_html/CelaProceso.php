<?php
	 ...
	 * Fecha: 12/11/2015
	 * Descripción: Controlador de la tabla "CelaProceso".
	 **/

	require_once('../Libraries/Functions.php');
	require_once('../Libraries/Session.class.php');
	require_once('../CelaProceso/CelaProceso.php');
	require_once('../Libraries/Connection.php');
	require_once('../Libraries/GetSession.php');
	require_once('../Libraries/Security.php');

	$ArgsCelaProcesoVistaLeer = array(
		'Table'             => 'CelaProceso',
		'ServerSource'      => '../CelaProceso/CelaProceso.php',
		'ServerFunction'    => 'CelaProcesoLeer',
		'RouteForm'         => $RouteForm
	);

	$ArgsCelaHeadContent['FormTitle'] = 'Listado de Procesos';
		...
		switch($_GET['Action']){
			case 'Crear':
				...
				if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Proceso';

					/*Se verifica que haya evento "submit"*/
					if((isset($_POST['CelaProcesoInsert'])) && ($_POST['CelaProcesoInsert'] == 'CelaProcesoInsert')){
						/*Se invoca la funcion crear*/
						$CelaProceso = CelaProcesoCrear($_POST);

						if($CelaProceso['Status'] == 'OK'){
							/*Se registra la acción "Crear" en la bitacora*/
							RecordLog('CelaProceso', $CelaProceso['idRecord'], 2, $SessionUserId, $_POST);
							$Status = true;
							...
							if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
								...
								$ArgsCelaProcesoVistaCrear = array(
									'SessionGroupId'    => $SessionGroupId,
									'Random'            => $SessionRandom,
									'FormAction'        => $RouteForm
								);

								$Content .= LoadContentPage('../CelaProceso/CelaProcesoVistaCrear.php', $ArgsCelaProcesoVistaCrear);
							}else{
								/*Se carga la vista de Lectura*/
								if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
									$ArgsCelaHeadContent['FormTitle'] = 'Listado de Procesos';

									$Content  .= LoadContentPage('../CelaProceso/CelaProcesoVistaLeer.php', $ArgsCelaProcesoVistaLeer);

									$ArgsCelaProcesoJavascript = array(
										'Table' => 'CelaProceso',
									);

									$MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsCelaProcesoJavascript);
								}
							}
						}else{
							$Status = false;
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => $CelaProceso['Error'].'<br />puede <a href="CelaProceso.php?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
							);
			...
			case 'Eliminar':
				...
				if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
					...
					foreach ($_GET['Key'] as $Key) {
						...
						$Data = GetValue(
									sprintf('SELECT * FROM CelaProceso WHERE `id` = %s;',
										GetSQLValueString($Key, 'int')
									)
								);
						$CelaProceso = CelaProcesoEliminar($Key);

						if($CelaProceso['Status'] == 'ERROR'){
							/*Se guarda el error para mostrarlo*/
							$Status = false;
							$BadKeys[]['Index'] = $Key;
							$BadKeys[]['Error'] = $CelaProceso['Error'];
						}else{
							/*Se registra la acción "Eliminar" en la bitacora*/
							RecordLog('CelaProceso', $Key, 3, $SessionUserId, $Data);
						}
					}
					if($Status){
						...
						$ArgsCelaActionMessage['TextMessage'] .= '<a href="CelaProceso.php"
	class="btn btn-danger">Aceptar</a>';
					...
					$Connection -> close();
					header(sprintf('Location: %s', 'CelaProceso.php'));
				}
				break;
			case 'Actualizar':
				/*Se verifica si se tiene el privilegio de actualizar*/
				if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Proceso';

					/*Se verifica que haya evento "submit"*/
					if(isset($_POST['CelaProcesoUpdate']) && $_POST['CelaProcesoUpdate'] == 'CelaProcesoUpdate'){
						...
						if($Status){
							...
							foreach($Keys as $Key){
								...
								$Data = array(
									...
									'Script' => $_POST['Script' . $Key],
									'Parametros' => $_POST['Parametros' . $Key],
									'Recurrente' => $_POST['Recurrente' . $Key],
									'Periodo' => $_POST['Periodo' . $Key],
									'Periodicidad' => $_POST['Periodicidad' . $Key],
									'FechaDeInicio' => $_POST['FechaDeInicio' . $Key],
									'FechaDeTermino' => $_POST['FechaDeTermino' . $Key]
								);
								$CelaProceso = CelaProcesoActualizar($Key, $Data);
								...
								if($CelaProceso['Status'] == 'ERROR'){
									...
									RecordLog('CelaProceso', $Key, 5, $SessionUserId, $Data);
								}
							}
	...
	class="btn btn-danger">Aceptar</a>';
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="CelaProceso.php"
	class="btn btn-danger">Aceptar</a>'
							);
							$Content = LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
						}
						...
						if(isset($_GET['Key']) && $_GET['Key'] != '') {
							...
							$ArgsCelaProcesoVistaActualizar = array(
								'SessionGroupId'    => $SessionGroupId,
								'Random'            => $SessionRandom,
								'FormAction'        => $RouteForm
							);
							$Content = LoadContentPage('../CelaProceso/CelaProcesoVistaActualizar.php', $ArgsCelaProcesoVistaActualizar);
						}else{
			...
			case 'Procesos':
				...
				if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
					$ArgsCelaProcesoVistaLeer = array(
						'Table'             => 'CelaProceso',
						'ServerSource'      => '../CelaProceso/CelaProceso.php',
						'ServerFunction'    => 'CelaProcesoBackend',
						'RouteForm'         => $RouteForm
					);
					
					$ArgsCelaHeadContent['FormTitle'] = 'Historial de Procesos';
					
					$Content  = LoadContentPage('../CelaProceso/CelaProcesoBackend.php', $ArgsCelaProcesoVistaLeer);
