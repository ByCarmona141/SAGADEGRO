<?php
	 ...
	 * Fecha: 12/11/2015
	 * Descripción: Controlador de la tabla "CelaTema".
	 **/
	...
	$ArgsCelaTemaVistaLeer = array(
		'Table'             => 'CelaTema',
		'ServerSource'      => '../CelaTema/CelaTema.php',
		'ServerFunction'    => 'CelaTemaLeer',
		'RouteForm'         => $RouteForm
	);

	$ArgsCelaHeadContent['FormTitle'] = 'Listado de Temas';
		...
		switch($_GET['Action']){
			case 'Crear':
				...
				if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Tema';

					/*Se verifica que haya evento "submit"*/
					if((isset($_POST['CelaTemaInsert'])) && ($_POST['CelaTemaInsert'] == 'CelaTemaInsert')){
						/*Se invoca la funcion crear*/
						$CelaTema = CelaTemaCrear($_POST);

						if($CelaTema['Status'] == 'OK'){
							/*Se registra la acción "Crear" en la bitacora*/
							RecordLog('CelaTema', $CelaTema['idRecord'], 2, $SessionUserId, $_POST);
							$Status = true;
							...
							if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
								...
								$ArgsCelaTemaVistaCrear = array(
									'SessionGroupId'    => $SessionGroupId,
									'Random'            => $SessionRandom,
									'FormAction'        => $RouteForm
								);

								$Content .= LoadContentPage('../CelaTema/CelaTemaVistaCrear.php', $ArgsCelaTemaVistaCrear);
							}else{
								if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
									$Content  .= LoadContentPage('../CelaTema/CelaTemaVistaLeer.php', $ArgsCelaTemaVistaLeer);

									$ArgsCelaTemaJavascript = array(
										'Table' => 'CelaTema',
									);

									$MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsCelaTemaJavascript);
								}
							}
						}else{
							$Status = false;
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => $CelaTema['Error'].'<br />puede <a href="CelaTema.php?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
							);
			...
			case 'Eliminar':
				...
				if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
					...
					foreach ($_GET['Key'] as $Key) {
						...
						$Data = GetValue(
									sprintf('SELECT * FROM CelaTema WHERE `id` = %s;',
										GetSQLValueString($Key, 'int')
									)
								);
						$CelaTema = CelaTemaEliminar($Key);

						if($CelaTema['Status'] == 'ERROR'){
							/*Se guarda el error para mostrarlo*/
							$Status = false;
							$Result = array();

							$Result['Index']    = $Key;
							$Result['Error']    = $CelaTema['Error'];
							$BadKeys[]          = $Result;
						}else{
							/*Se registra la acción "Eliminar" en la bitacora*/
							RecordLog('CelaTema', $Key, 3, $SessionUserId, $Data);
						}
					}
					if($Status){
						...
						$ArgsCelaActionMessage['TextMessage'] .= '<a href="CelaTema.php"
	class="btn btn-danger">Aceptar</a>';
					...
					$Connection -> close();
					header(sprintf('Location: %s', 'CelaTema.php'));
				}
				break;
			case 'Actualizar':
				/*Se verifica si se tiene el privilegio de actualizar*/
				if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Tema';

					/*Se verifica que haya evento "submit"*/
					if(isset($_POST['CelaTemaUpdate']) && $_POST['CelaTemaUpdate'] == 'CelaTemaUpdate'){
						...
						if($Status){
							...
							foreach($Keys as $Key){
								...
								$Data = array(
									...
									'Ruta'      => $_POST['Ruta' . $Key],
									'Imagen'    => $_POST['Imagen' . $Key]
								);
								$CelaTema = CelaTemaActualizar($Key, $Data);
								...
								if($CelaTema['Status'] == 'ERROR'){
									...
									RecordLog('CelaTema', $Key, 5, $SessionUserId, $Data);
								}
							}
	...
	class="btn btn-danger">Aceptar</a>';
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="CelaTema.php"
	class="btn btn-danger">Aceptar</a>'
							);
							$Content = LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
						}
						...
						if(isset($_GET['Key']) && $_GET['Key'] != '') {
							...
							$ArgsCelaTemaVistaActualizar = array(
								'SessionGroupId'    => $SessionGroupId,
								'Random'            => $SessionRandom,
								'FormAction'        => $RouteForm
							);
							$Content = LoadContentPage('../CelaTema/CelaTemaVistaActualizar.php', $ArgsCelaTemaVistaActualizar);
						}else{
