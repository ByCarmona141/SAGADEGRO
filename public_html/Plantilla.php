<?php
	...
	require_once('../Opci_on/Opci_on.php');
	require_once('../Plantilla/Plantilla.php');
	require_once('../Libraries/Connection.php');
	require_once('../Libraries/GetSession.php');
	require_once('../Libraries/Security.php');

	$ArgsPlantillaVistaLeer = array(
		'Table'             => 'Plantilla',
		'ServerSource'      => '../Plantilla/Plantilla.php',
		'ServerFunction'    => 'PlantillaLeer',
		'RouteForm'         => $RouteForm
	);

	$ArgsCelaHeadContent['FormTitle'] = 'Listado de Plantilla';
		...
		switch($_GET['Action']){
			case 'Crear':
				...
				if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Plantilla';

					/*Se verifica que haya evento "submit"*/
					if((isset($_POST['PlantillaInsert'])) && ($_POST['PlantillaInsert'] == 'PlantillaInsert')){
						/*Se invoca la funcion crear*/
						$Data = array(
							'Nombre' => $_POST['Nombre'],
							'Descripci_on' => $_POST['Descripci_on'],
							'Plantilla' => $_POST['Plantilla'],
							'TipoPlantilla' => $_POST['TipoPlantilla'],
							'EstaVigente' => $_POST['EstaVigente'],
							'Tama_no' => $_POST['Tama_no']
						);
						$Plantilla = PlantillaCrear($Data);

						if($Plantilla['Status'] == 'OK'){
							

							/*Se registra la acción "Crear" en la bitacora*/
							RecordLog('Plantilla', $Plantilla['idRecord'], 2, $SessionUserId, $Data);
							$Status = true;
							...
							if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
								...
								$ArgsPlantillaVistaCrear = array(
									'SessionGroupId'    => $SessionGroupId,
									'Random'            => $SessionRandom,
									'FormAction'        => $RouteForm
								);

								$Content .= LoadContentPage('../Plantilla/PlantillaVistaCrear.php', $ArgsPlantillaVistaCrear);
							}else{
								/*Se carga la vista de Leer*/
								if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
									$ArgsCelaHeadContent['FormTitle'] = 'Listado de Plantilla';
									$Content  .= LoadContentPage('../Plantilla/PlantillaVistaLeer.php', $ArgsPlantillaVistaLeer);

									$ArgsPlantillaJavascript = array(
										'Table' => 'Plantilla',
									);

									$MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsPlantillaJavascript);
								}
							}
						}else{
							$Status = false;
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => $Plantilla['Error'].'<br />puede <a href="Plantilla?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
							);
			...
			case 'Eliminar':
				...
				if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
					...
					foreach ($_GET['Key'] as $Key) {
						...
						$Plantilla = PlantillaEliminar($Key);

						if($Plantilla['Status'] == 'ERROR'){
							/*Se guarda el error para mostrarlo*/
							$Status = false;
							$Result = array(
								'Index' => $Key,
								'Error' => $Plantilla['Error']
							);

							$BadKeys[] = $Result;
						}else{
							/*Se registra la acción "Eliminar" en la bitacora*/
							RecordLog('Plantilla', $Key, 3, $SessionUserId, $Data);
						}
					}
					if($Status){
						...
						$ArgsCelaActionMessage['TextMessage'] .= '<a href="Plantilla"
	class="btn btn-danger">Aceptar</a>';
					...
					$Connection -> close();
					header(sprintf('Location: %s', 'Plantilla'));
				}
				break;
			case 'Actualizar':
				/*Se verifica si se tiene el privilegio de actualizar*/
				if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Plantilla';

					/*Se verifica que haya evento "submit"*/
					if(isset($_POST['PlantillaUpdate']) && $_POST['PlantillaUpdate'] == 'PlantillaUpdate'){
						...
						if($Status){
							...
							foreach($Keys as $Key){
								...
								$Data = array(
									...
									'Plantilla' => $_POST['Plantilla' . $Key],
									'TipoPlantilla' => $_POST['TipoPlantilla' . $Key],
									'EstaVigente' => $_POST['EstaVigente' . $Key],
									'Tama_no' => $_POST['Tama_no' . $Key]
								);
								$Plantilla = PlantillaActualizar($Key, $Data);
								...
								if($Plantilla['Status'] == 'ERROR'){
									...
									RecordLog('Plantilla', $Key, 5, $SessionUserId, $Data);
								}
							}
	...
	class="btn btn-danger">Aceptar</a>';
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="Plantilla"
	class="btn btn-danger">Aceptar</a>'
							);
							$Content .= LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
						}
						...
						if(isset($_GET['Key']) && $_GET['Key'] != '') {
							...
							$ArgsPlantillaVistaActualizar = array(
								'SessionGroupId'    => $SessionGroupId,
								'Random'            => $SessionRandom,
								'FormAction'        => $RouteForm
							);
							$Content .= LoadContentPage('../Plantilla/PlantillaVistaActualizar.php', $ArgsPlantillaVistaActualizar);
						}else{
