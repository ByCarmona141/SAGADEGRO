...
$ArgsUbicacionVistaLeer = array(
    'Table'             => 'Ubicacion',
    'ServerSource'      => '../Ubicacion/Ubicacion.php',
    'ServerFunction'    => 'UbicacionLeer',
    'RouteForm'         => $RouteForm
);

$ArgsCelaHeadContent['FormTitle'] = 'Listado de Ubicaciones';
...
if(isset($_GET['Action'])) {
    switch($_GET['Action']) {
        case 'Crear':
            ...
            if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Ubicacion';

                /*Se verifica que haya evento "submit"*/
                if((isset($_POST['UbicacionInsert'])) && ($_POST['UbicacionInsert'] == 'UbicacionInsert')) {
                    /*Se invoca la funcion crear*/
                    $Data = array(
                        'Area' => $_POST['Area'],
                        'Piso' => $_POST['Piso']
                    );
                    $Ubicacion = UbicacionCrear($Data);

                    if($Ubicacion['Status'] == 'OK') {
                        /*Se registra la acción "Crear" en la bitacora*/
                        RecordLog('Ubicacion', $Ubicacion['idRecord'], 2, $SessionUserId, $Data);
                        $Status = true;
                        ...
                        if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
                            ...
                            $ArgsUbicacionVistaCrear = array(
                                'SessionGroupId'    => $SessionGroupId,
                                'Random'            => $SessionRandom,
                                'FormAction'        => $RouteForm
                            );

                            $Content .= LoadContentPage('../Ubicacion/UbicacionCrear.php', $ArgsUbicacionVistaCrear);
                        }else{
                            /*Se carga la vista de Leer*/
                            if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
                                $ArgsCelaHeadContent['FormTitle'] = 'Listado de Ubicaciones';
                                $Content  .= LoadContentPage('../Ubicacion/UbicacionLeer.php', $ArgsUbicacionVistaLeer);

                                $ArgsUbicacionJavascript = array(
                                    'Table' => 'Ubicacion',
                                );

                                $MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsUbicacionJavascript);
                            }
                        }
                    }else{
                        $Status = false;
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => $Ubicacion['Error'].'<br />puede <a href="Ubicacion?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
                        );
        ...
        case 'Eliminar':
            ...
            if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
                ...
                foreach ($_GET['Key'] as $Key) {
                    ...
                    $Data = GetValue(
                        sprintf('SELECT * FROM Ubicacion WHERE `id` = %s;',
                            GetSQLValueString($Key, 'tinyint unsigned')
                        )
                    );
                    $Ubicacion = UbicacionEliminar($Key);

                    if($Ubicacion['Status'] == 'ERROR'){
                        ...
                        RecordLog('Ubicacion', $Key, 3, $SessionUserId, $Data);
                    }
                }
                if($Status){
                    ...
                    $ArgsCelaActionMessage['TextMessage'] .= '<a href="Ubicacion"
	class="btn btn-danger">Aceptar</a>';
                ...
                $Connection -> close();
                header(sprintf('Location: %s', 'Ubicacion'));
            }
            break;
        case 'Actualizar':
            /*Se verifica si se tiene el privilegio de actualizar*/
            if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Ubicacion';

                /*Se verifica que haya evento "submit"*/
                if(isset($_POST['UbicacionUpdate']) && $_POST['UbicacionUpdate'] == 'UbicacionUpdate'){
                    ...
                    if($Status) {
                        ...
                        foreach($Keys as $Key) {
                            ...
                            $Ubicacion = UbicacionActualizar($Key, $Data);
                            ...
                            if($Ubicacion['Status'] == 'ERROR') {
                                ...
                                RecordLog('Ubicacion', $Key, 5, $SessionUserId, $Data);
                            }
                        }
	...
	class="btn btn-danger">Aceptar</a>';
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="Ubicacion"
	class="btn btn-danger">Aceptar</a>'
                        );
                        $Content .= LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
                    }
                    ...
                    if(isset($_GET['Key']) && $_GET['Key'] != '') {
                        ...
                        $ArgsUbicacionVistaActualizar = array(
                            'SessionGroupId'    => $SessionGroupId,
                            'Random'            => $SessionRandom,
                            'FormAction'        => $RouteForm
                        );
                        $Content .= LoadContentPage('../Ubicacion/UbicacionActualizar.php', $ArgsUbicacionVistaActualizar);
                    }else{
