<?php
	 ...
	 * Fecha: 12/11/2015
	 * Descripción: Controlador de la tabla "CelaTipoComponente".
	 **/
	...
	$ArgsCelaTipoComponenteVistaLeer = array(
		'Table'             => 'CelaTipoComponente',
		'ServerSource'      => '../CelaTipoComponente/CelaTipoComponente.php',
		'ServerFunction'    => 'CelaTipoComponenteLeer',
		'RouteForm'         => $RouteForm
	);

	$ArgsCelaHeadContent['FormTitle'] = 'Listado de Tipos de Componentes';
		...
		switch($_GET['Action']){
			case 'Crear':
				...
				if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Tipo de Componente';

					/*Se verifica que haya evento "submit"*/
					if((isset($_POST['CelaTipoComponenteInsert'])) && ($_POST['CelaTipoComponenteInsert'] == 'CelaTipoComponenteInsert')){
						/*Se invoca la funcion crear*/
						$CelaTipoComponente = CelaTipoComponenteCrear($_POST);

						if($CelaTipoComponente['Status'] == 'OK'){
							/*Se registra la acción "Crear" en la bitacora*/
							RecordLog('CelaTipoComponente', $CelaTipoComponente['idRecord'], 2, $SessionUserId, $_POST);
							$Status = true;
							...
							if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
								...
								$ArgsCelaTipoComponenteVistaCrear = array(
									'SessionGroupId'    => $SessionGroupId,
									'Random'            => $SessionRandom,
									'FormAction'        => $RouteForm
								);

								$Content .= LoadContentPage('../CelaTipoComponente/CelaTipoComponenteVistaCrear.php', $ArgsCelaTipoComponenteVistaCrear);
							}else{
								if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
									$Content  .= LoadContentPage('../CelaTipoComponente/CelaTipoComponenteVistaLeer.php', $ArgsCelaTipoComponenteVistaLeer);

									$ArgsCelaTipoComponenteJavascript = array(
										'Table' => 'CelaTipoComponente',
									);

									$MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsCelaTipoComponenteJavascript);
								}
							}
						}else{
							$Status = false;
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => $CelaTipoComponente['Error'].'<br />puede <a href="CelaTipoComponente.php?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
							);
			...
			case 'Eliminar':
				...
				if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
					...
					foreach ($_GET['Key'] as $Key) {
						...
						$Data = GetValue(
									sprintf('SELECT * FROM CelaTipoComponente WHERE `id` = %s;',
										GetSQLValueString($Key, 'int')
									)
								);
						$CelaTipoComponente = CelaTipoComponenteEliminar($Key);

						if($CelaTipoComponente['Status'] == 'ERROR'){
							/*Se guarda el error para mostrarlo*/
							$Status = false;
							$Result = array();

							$Result['Index']    = $Key;
							$Result['Error']    = $CelaTipoComponente['Error'];
							$BadKeys[]          = $Result;
						}else{
							/*Se registra la acción "Eliminar" en la bitacora*/
							RecordLog('CelaTipoComponente', $Key, 3, $SessionUserId, $Data);
						}
					}
					if($Status){
						...
						$ArgsCelaActionMessage['TextMessage'] .= '<a href="CelaTipoComponente.php"
	class="btn btn-danger">Aceptar</a>';
					...
					$Connection -> close();
					header(sprintf('Location: %s', 'CelaTipoComponente.php'));
				}
				break;
			case 'Actualizar':
				/*Se verifica si se tiene el privilegio de actualizar*/
				if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Tipo de Componente';

					/*Se verifica que haya evento "submit"*/
					if(isset($_POST['CelaTipoComponenteUpdate']) && $_POST['CelaTipoComponenteUpdate'] == 'CelaTipoComponenteUpdate'){
						...
						if($Status){
							...
							foreach($Keys as $Key){
								...
								$CelaTipoComponente = CelaTipoComponenteActualizar($Key, $Data);
								...
								if($CelaTipoComponente['Status'] == 'ERROR'){
									...
									RecordLog('CelaTipoComponente', $Key, 5, $SessionUserId, $Data);
								}
							}
	...
	class="btn btn-danger">Aceptar</a>';
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="CelaTipoComponente.php"
	class="btn btn-danger">Aceptar</a>'
							);
							$Content = LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
						}
						...
						if(isset($_GET['Key']) && $_GET['Key'] != '') {
							...
							$ArgsCelaTipoComponenteVistaActualizar = array(
								'SessionGroupId'    => $SessionGroupId,
								'Random'            => $SessionRandom,
								'FormAction'        => $RouteForm
							);
							$Content .= LoadContentPage('../CelaTipoComponente/CelaTipoComponenteVistaActualizar.php', $ArgsCelaTipoComponenteVistaActualizar);
						}else{
