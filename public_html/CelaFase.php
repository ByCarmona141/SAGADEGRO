<?php
	 ...
	 * Fecha: 12/11/2015
	 * Descripción: Controlador de la tabla "CelaFase".
	 **/

	require_once('../Libraries/Functions.php');
	require_once('../Libraries/Session.class.php');
	require_once('../CelaFase/CelaFase.php');
	require_once('../Libraries/Connection.php');
	require_once('../Libraries/GetSession.php');
	require_once('../Libraries/Security.php');

	$ArgsCelaFaseVistaLeer = array(
		'Table'             => 'CelaFase',
		'ServerSource'      => '../CelaFase/CelaFase.php',
		'ServerFunction'    => 'CelaFaseLeer',
		'RouteForm'         => $RouteForm
	);

	$ArgsCelaHeadContent['FormTitle'] = 'Listado de Faces';
		...
		switch($_GET['Action']){
			case 'Crear':
				...
				if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Face';

					/*Se verifica que haya evento "submit"*/
					if((isset($_POST['CelaFaseInsert'])) && ($_POST['CelaFaseInsert'] == 'CelaFaseInsert')){
						/*Se invoca la funcion crear*/
						$CelaFase = CelaFaseCrear($_POST);

						if($CelaFase['Status'] == 'OK'){
							/*Se registra la acción "Crear" en la bitacora*/
							RecordLog('CelaFase', $CelaFase['idRecord'], 2, $SessionUserId, $_POST);
							$Status = true;
							...
							if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
								...
								$ArgsCelaFaseVistaCrear = array(
									'SessionGroupId'    => $SessionGroupId,
									'Random'            => $SessionRandom,
									'FormAction'        => $RouteForm
								);

								$Content = LoadContentPage('../CelaFase/CelaFaseVistaCrear.php', $ArgsCelaFaseVistaCrear);
							}else{
								if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
									$Content  .= LoadContentPage('../CelaFase/CelaFaseVistaLeer.php', $ArgsCelaFaseVistaLeer);

									$ArgsCelaFaseJavascript = array(
										'Table' => 'CelaFase',
									);

									$MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsCelaFaseJavascript);
								}
							}
						}else{
							$Status = false;
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => $CelaFase['Error'].'<br />puede <a href="CelaFase.php?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
							);
			...
			case 'Eliminar':
				...
				if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
					...
					foreach ($_GET['Key'] as $Key) {
						...
						$Data = GetValue(
									sprintf('SELECT * FROM CelaFase WHERE `id` = %s;',
										GetSQLValueString($Key, 'int')
									)
								);
						$CelaFase = CelaFaseEliminar($Key);

						if($CelaFase['Status'] == 'ERROR'){
							/*Se guarda el error para mostrarlo*/
							$Status = false;
							$Result = array();

							$Result['Index']    = $Key;
							$Result['Error']    = $CelaFase['Error'];
							$BadKeys[]          = $Result;
						}else{
							/*Se registra la acción "Eliminar" en la bitacora*/
							RecordLog('CelaFase', $Key, 3, $SessionUserId, $Data);
						}
					}
					if($Status){
						...
						$ArgsCelaActionMessage['TextMessage'] .= '<a href="CelaFase.php"
	class="btn btn-danger">Aceptar</a>';
					...
					$Connection -> close();
					header(sprintf('Location: %s', 'CelaFase.php'));
				}
				break;
			case 'Actualizar':
				/*Se verifica si se tiene el privilegio de actualizar*/
				if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
					$ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Face';

					/*Se verifica que haya evento "submit"*/
					if(isset($_POST['CelaFaseUpdate']) && $_POST['CelaFaseUpdate'] == 'CelaFaseUpdate'){
						...
						if($Status){
							...
							foreach($Keys as $Key){
								...
								$CelaFase = CelaFaseActualizar($Key, $Data);
								...
								if($CelaFase['Status'] == 'ERROR'){
									...
									RecordLog('CelaFase', $Key, 5, $SessionUserId, $Data);
								}
							}
	...
	class="btn btn-danger">Aceptar</a>';
							...
							$ArgsCelaActionMessage = array(
								...
								'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="CelaFase.php"
	class="btn btn-danger">Aceptar</a>'
							);
							$Content = LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
						}
						...
						if(isset($_GET['Key']) && $_GET['Key'] != '') {
							...
							$ArgsCelaFaseVistaActualizar = array(
								'SessionGroupId'    => $SessionGroupId,
								'Random'            => $SessionRandom,
								'FormAction'        => $RouteForm
							);
							$Content = LoadContentPage('../CelaFase/CelaFaseVistaActualizar.php', $ArgsCelaFaseVistaActualizar);
						}else{
