...
$ArgsMarcaVistaLeer = array(
    'Table'             => 'Marca',
    'ServerSource'      => '../Marca/Marca.php',
    'ServerFunction'    => 'MarcaLeer',
    'RouteForm'         => $RouteForm
);

$ArgsCelaHeadContent['FormTitle'] = 'Listado de Marcas';
...
if(isset($_GET['Action'])) {
    switch($_GET['Action']) {
        case 'Crear':
            ...
            if(isset($Privileges['Crear']) && $Privileges['Crear'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Creaci&oacute;n de Marca';

                /*Se verifica que haya evento "submit"*/
                if((isset($_POST['MarcaInsert'])) && ($_POST['MarcaInsert'] == 'MarcaInsert')) {
                    /*Se invoca la funcion crear*/
                    $Data = array(
                        'Nombre' => strtoupper($_POST['Nombre'])
                    );
                    $Marca = MarcaCrear($Data);

                    if($Marca['Status'] == 'OK') {
                        /*Se registra la acción "Crear" en la bitacora*/
                        RecordLog('Marca', $Marca['idRecord'], 2, $SessionUserId, $Data);
                        $Status = true;
                        ...
                        if(isset($_POST['InsertBack']) && $_POST['InsertBack'] == 1){
                            ...
                            $ArgsMarcaVistaCrear = array(
                                'SessionGroupId'    => $SessionGroupId,
                                'Random'            => $SessionRandom,
                                'FormAction'        => $RouteForm
                            );

                            $Content .= LoadContentPage('../Marca/MarcaCrear.php', $ArgsMarcaVistaCrear);
                        }else{
                            /*Se carga la vista de Leer*/
                            if(isset($Privileges['Leer']) && $Privileges['Leer'] == 1) {
                                $ArgsCelaHeadContent['FormTitle'] = 'Listado de Marcas';
                                $Content  .= LoadContentPage('../Marca/MarcaLeer.php', $ArgsMarcaVistaLeer);

                                $ArgsMarcaJavascript = array(
                                    'Table' => 'Marca',
                                );

                                $MyScripts .= LoadContentPage('../CelaTemplate/CelaTableToolsScript.php', $ArgsMarcaJavascript);
                            }
                        }
                    }else{
                        $Status = false;
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => $Marca['Error'].'<br />puede <a href="Marca?' . EncodeThis('Action=Crear') . '" class="alert-link">volver a intentar</a> &oacute; <a href="Escritorio.php" class="alert-link">ir al escritorio</a>'
                        );
        ...
        case 'Eliminar':
            ...
            if(isset($_GET['Key']) && $_GET['Key'] != '' && isset($Privileges['Eliminar']) && $Privileges['Eliminar'] == 1) {
                ...
                foreach ($_GET['Key'] as $Key) {
                    ...
                    $Data = GetValue(
                        sprintf('SELECT * FROM Marca WHERE `id` = %s;',
                            GetSQLValueString($Key, 'tinyint unsigned')
                        )
                    );
                    $Marca = MarcaEliminar($Key);

                    if($Marca['Status'] == 'ERROR'){
                        ...
                        RecordLog('Marca', $Key, 3, $SessionUserId, $Data);
                    }
                }
                if($Status){
                    ...
                    $ArgsCelaActionMessage['TextMessage'] .= '<a href="Marca"
	class="btn btn-danger">Aceptar</a>';
                ...
                $Connection -> close();
                header(sprintf('Location: %s', 'Marca'));
            }
            break;
        case 'Actualizar':
            /*Se verifica si se tiene el privilegio de actualizar*/
            if(isset($Privileges['Actualizar']) && $Privileges['Actualizar'] == 1) {
                $ArgsCelaHeadContent['FormTitle'] = 'Actualizaci&oacute;n de Marcas';

                /*Se verifica que haya evento "submit"*/
                if(isset($_POST['MarcaUpdate']) && $_POST['MarcaUpdate'] == 'MarcaUpdate'){
                    ...
                    if($Status) {
                        ...
                        foreach($Keys as $Key) {
                            ...
                            $Marca = MarcaActualizar($Key, $Data);
                            ...
                            if($Marca['Status'] == 'ERROR') {
                                ...
                                RecordLog('Marca', $Key, 5, $SessionUserId, $Data);
                            }
                        }
	...
	class="btn btn-danger">Aceptar</a>';
                        ...
                        $ArgsCelaActionMessage = array(
                            ...
                            'TextMessage'   => 'Algunos elementos para actualizar estan corruptos o no existen. <a href="Marca"
	class="btn btn-danger">Aceptar</a>'
                        );
                        $Content .= LoadContentPage('../CelaTemplate/CelaActionMessage.php', $ArgsCelaActionMessage);
                    }
                    ...
                    if(isset($_GET['Key']) && $_GET['Key'] != '') {
                        ...
                        $ArgsMarcaVistaActualizar = array(
                            'SessionGroupId'    => $SessionGroupId,
                            'Random'            => $SessionRandom,
                            'FormAction'        => $RouteForm
                        );
                        $Content .= LoadContentPage('../Marca/MarcaActualizar.php', $ArgsMarcaVistaActualizar);
                    }else{
